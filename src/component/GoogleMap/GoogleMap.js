...
const GoogleMapView = ({
  ...
  useEffect(() => {
    ...
    if (map === null && mapDivRef.current !== null) {
      ...
      dragStartListener = google.maps.event.addListener(
        map,
        "dragstart",
        () => {
          setIsDragging(true);
  ...
  return (
    <div style={{ width, height }} className='GoogleMapView__container'>
      <div style={{ width, height }} ref={mapDivRef} id='map'></div>
      <img
        id='ic-location'
      ...
      <i
        className={`ic-circle ${
          isDragging ? "ic-circle-up" : "ic-circle-down"
        }`}
      ></i>
      {showMyLocationButton && latestLocation && (
        ...
        >
          <img src={icMyLocation} alt='sayurbox-ic-my-location' />
        </div>
      )}
    </div>
  );
